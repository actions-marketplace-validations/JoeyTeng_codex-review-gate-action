...
源码仓库也在 `.github/workflows/codex-review-gate.yml` 中为自己的 PR 启用了 `codex/review-gate`。因为该 workflow 是 privileged `pull_request_target` automation，它会 checkout `github.event.repository.default_branch`，再通过 `uses: ./packages/action` 调用可信 default branch 上的源码包；不会运行 PR 提供的 action 代码，也不依赖可移动 release tag。

Marketplace 仓库由源码仓库的 `packages/action` subtree 生成；发布后这个 package 会成为 repository root。
